...
This branch adds an **experimental**, disabled-by-default `debugMode` option to the precomputed browser provider. It is not a production telemetry release. The backend experiment currently accepts diagnostics only for explicitly supported staging test traffic.
