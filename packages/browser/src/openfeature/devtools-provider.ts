import type { EvaluationContext, JsonValue, Logger, Paradigm, Provider, ProviderMetadata, ResolutionDetails } from '@openfeature/web-sdk'
...
export class DevToolsProvider implements Provider {
  ...
  resolveBooleanEvaluation(flagKey: string, _defaultValue: boolean, _context: EvaluationContext, _logger: Logger): ResolutionDetails<boolean> {
    return resolveOverride<boolean>(flagKey, ['BOOLEAN'])
  }

  resolveStringEvaluation(flagKey: string, _defaultValue: string, _context: EvaluationContext, _logger: Logger): ResolutionDetails<string> {
    return resolveOverride<string>(flagKey, ['STRING'])
  }

  resolveNumberEvaluation(flagKey: string, _defaultValue: number, _context: EvaluationContext, _logger: Logger): ResolutionDetails<number> {
    return resolveOverride<number>(flagKey, ['INTEGER', 'NUMERIC'])
  }

  resolveObjectEvaluation<T extends JsonValue>(flagKey: string, _defaultValue: T, _context: EvaluationContext, _logger: Logger): ResolutionDetails<T> {
