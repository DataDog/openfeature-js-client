...
describe('protobuf shard attributes', () => {
  const logger: Logger = {
    error: jest.fn(),
    warn: jest.fn(),
    info: jest.fn(),
  ...
  it('does not match when a named shard attribute is missing', () => {
    expect(evaluateForSubject(flag, 'boolean', 'user', {}, false, logger)).toMatchObject({
      value: false,
      reason: 'DEFAULT',
    })
  })

  it('uses the separately supplied targeting key for an explicit targetingKey attribute', () => {
    const targetingKeyFlag: Flag = {
      ...flag,
      allocations: [
        {
          ...flag.allocations[0],
          splits: [
            {
              ...flag.allocations[0].splits[0],
              shards: [{ ...flag.allocations[0].splits[0].shards[0], attribute: 'targetingKey' }],
            },
          ],
        },
      ],
    ...
    expect(evaluateForSubject(targetingKeyFlag, 'boolean', 'user', {}, false, logger)).toMatchObject({
      value: true,
      reason: 'TARGETING_MATCH',
    })
  })
